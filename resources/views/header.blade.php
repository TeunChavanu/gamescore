<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    @vite('resources/css/app.css')
</head>

<body>
    <header class="flex justify-between items-center p-4 bg-white shadow-md">
        <h1 class="text-4xl text-blue-500 font-bold">Gamescore</h1>
        <nav>
            <ul class="flex gap-6 items-center">
                <a class="text-gray-700 hover:text-blue-500 transition font-medium" href="/">Home</a>
                <a class="text-gray-700 hover:text-blue-500 transition font-medium" href="/all_games">Games</a>
                @if(Auth::check())
                <a class="text-gray-700 hover:text-blue-500 transition font-medium" href="/review">Review</a>
                @if(auth()->user()->role === 'admin')
                <a class="text-gray-700 hover:text-blue-500 transition font-medium" href="/add">Add</a>
                @endif
                <div class="relative">
                    <button onclick="toggleMenu()" class="flex items-center gap-2 bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300 transition">
                        <span>{{ auth()->user()->name }}</span>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>

                    <div id="userMenu" class="hidden absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-lg">
                        <a href="/profile" class="block px-4 py-2 hover:bg-gray-100">Profile</a>

                        @if(auth()->user()->role === 'admin')
                        <a href="/admin" class="block px-4 py-2 hover:bg-gray-100">Admin Dashboard</a>
                        @endif

                        <form method="POST" action="/logout">
                            @csrf
                            <button type="submit" class="w-full text-left px-4 py-2 hover:bg-gray-100">Logout</button>
                        </form>
                    </div>
                </div>

                <script>
                    function toggleMenu() {
                        document.getElementById('userMenu').classList.toggle('hidden');
                    }
                </script>

                @else
                <a href="/login" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition font-medium">
                    Login
                </a>
                @endif
            </ul>
        </nav>
    </header>
</body>

</html>
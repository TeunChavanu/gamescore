...
<body class="bg-gray-100">
    ...
    $games = Game::all()->sortByDesc('rating')->take(5);
    ?>
   <div class="grid grid-cols-5 gap-3 mx-7 mb-8">
    @foreach ($games as $game)
        <div class="flex flex-col bg-white p-2 mb-4 rounded-lg shadow-lg w-5/6 mx-auto">
            <!-- Afbeelding iets kleiner zodat het in de card past -->
            ...
            <!-- Genre -->
            <p class="flex grow text-gray-600 truncate">{{ $game->genre }}</p>  
            ...
            <div class="flex items-center text-yellow-500 justify-between mt-1">
                <div class="flex items-center gap-1">
                    ...
                    <span>{{ trim(trim($game->rating, '0'), '.') ?? 'N/A' }}</span>
                </div>
                <a class="text-blue-500 hover:text-blue-700 text-xs" href="/games/{{ $game->id }}">View</a>
            </div>
        </div>
    @endforeach
</div>
